...
export function CalendarImportDialog({ open, preferredConnectionId, onClose, onChoose, embedded = false }: { open: boolean; preferredConnectionId?: string | null; onClose(): void; onChoose(selection: CalendarSelection): void; embedded?: boolean }) {
  ...
  const content = <>
      ...
      <div className="calendar-provider-grid" aria-label="Calendar connections">
        ...
        <div className={calendlyCount ? "calendar-provider-card connected" : "calendar-provider-card"}><CalendarBrandIcon provider="calendly" /><div><b>Calendly</b><small>{calendlyCount ? `${calendlyCount} connected account${calendlyCount === 1 ? "" : "s"}` : "Booked events and invitees"}</small></div><div className="calendar-provider-actions">{calendlyCount ? <span className="calendar-connection-badge"><CheckCircle2 /> Connected</span> : null}<button className="button secondary" type="button" disabled={busy || loadingConnections} onClick={() => void connect("calendly")}>{calendlyCount ? "Add another" : "Connect account"}</button></div></div>
        <div className={zoomCount ? "calendar-provider-card connected" : "calendar-provider-card"}><CalendarBrandIcon provider="zoom" /><div><b>Zoom</b><small>{zoomCount ? `${zoomCount} connected account${zoomCount === 1 ? "" : "s"}` : "Hosted upcoming meetings"}</small></div><div className="calendar-provider-actions">{zoomCount ? <span className="calendar-connection-badge"><CheckCircle2 /> Connected</span> : null}<button className="button secondary" type="button" disabled={busy || loadingConnections} onClick={() => void connect("zoom")}>{zoomCount ? "Add another" : "Connect account"}</button></div></div>
      </div>
