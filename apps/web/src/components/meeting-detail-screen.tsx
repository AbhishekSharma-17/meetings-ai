...
export function MeetingDetailScreen({ meetingId, focusSegmentId, onBack, onMeetingChange, onDeleted }: { meetingId: string; focusSegmentId?: string | null; onBack(): void; onMeetingChange(meeting: MeetingDetail): void; onDeleted(id: string): void }) {
  ...
  return <section className="page detail-page">
    ...
    <div className="lifecycle-banner" role="status"><span className={`status ${meeting.status}`}>{schedule?.status === "pending" ? "Scheduled" : lifecycle.label}</span><div><b>{schedule?.status === "pending" ? "The assistant is queued for this calendar event." : lifecycle.detail}</b><p>Controls operate on this product meeting record; the persisted capture identifier is resolved by the API.</p></div></div>
