...
import type { CalendarEvent, CalendarSchedule, KnowledgeBase, MeetingDetail, MeetingParticipants, SpeakerIdentity, TranscriptSegment, TranscriptionRoute } from "@/lib/types";
...
export function MeetingDetailScreen({ meetingId, focusSegmentId, onBack, onMeetingChange, onDeleted }: { meetingId: string; focusSegmentId?: string | null; onBack(): void; onMeetingChange(meeting: MeetingDetail): void; onDeleted(id: string): void }) {
  ...
  return <section className="page detail-page">
    ...
    {schedule ? <div className="calendar-schedule-banner" role="status"><div><b>{schedule.provider === "manual" ? "Scheduled assistant" : "Calendar assistant"} · {schedule.status}</b><p>Meeting starts {formatTimestamp(schedule.starts_at)}. {schedule.status === "pending" ? "The assistant will join at the scheduled start time. You can cancel the automatic join without deleting this meeting." : schedule.last_error || "Review this meeting for capture updates."}</p></div>{schedule.status === "pending" ? <button className="button secondary" type="button" onClick={() => void meetingsService.cancelCalendarSchedule(meetingId).then(setSchedule).catch((cause) => setError(cause instanceof Error ? cause.message : "Could not cancel the scheduled join."))}>Cancel auto-join</button> : null}</div> : null}
    {source ? <section className="detail-card meeting-source-card"><div><p className="eyebrow">CONNECTED SOURCE</p><h2>{source.provider === "googlecalendar" ? "Google Calendar" : source.provider === "outlook" ? "Outlook Calendar" : source.provider === "calendly" ? "Calendly" : "Zoom"}</h2><p>{new Date(source.starts_at).toLocaleString()} · {source.invitees?.length ?? 0} listed invitees</p></div>{source.organizer ? <p><b>Organizer</b> · {source.organizer}</p> : null}{source.agenda ? <p><b>Agenda</b> · {source.agenda}</p> : null}<small>Source details are saved at import time. The invitee list is not proof of attendance.</small></section> : null}
    ...
    <div className="lifecycle-banner" role="status"><span className={`status ${meeting.status}`}>{schedule?.status === "pending" ? "Scheduled" : lifecycle.label}</span><div><b>{schedule?.status === "pending" ? "The assistant is queued for this meeting." : lifecycle.detail}</b><p>Controls operate on this product meeting record; the persisted capture identifier is resolved by the API.</p></div></div>
    ...
    <div className="detail-grid">
      ...
      <section className="detail-card" aria-labelledby="capture-title"><h2 id="capture-title">People & capture</h2><p className="muted-copy">Duration: {meeting.duration}</p>{participants?.participants.length ? <ul className="participant-list">{participants.participants.map((person, index) => <li key={`${person.source}-${person.name}-${index}`}><b>{person.name}</b> <span>{person.source === "invite" ? "Invited" : "Heard speaking"}</span>{person.email ? <small>{person.email}</small> : null}</li>)}</ul> : <p className="muted-copy">No participant evidence is available yet.</p>}<p className="muted-copy">This is not a verified attendance roster. Invitees may not have joined; silent attendees may be absent. Speaker names are not automatically matched to invitee emails.</p>{namedSpeakers.length ? <div className="speaker-identities"><h3>Confirm speaker contact</h3><p>Only link an email after you verify who spoke. This does not add them to recap recipients.</p>{namedSpeakers.map((speaker) => { const identity = speakerIdentities.find((item) => item.speaker === speaker); const suggestion = source?.invitees?.find((person) => person.email && person.name.toLocaleLowerCase() === speaker.toLocaleLowerCase()); return <div key={speaker} className="speaker-identity-row"><span><b>{speaker}</b>{identity ? ` · ${identity.email} (confirmed)` : " · email not linked"}{!identity && suggestion ? <small>Possible invitee: {suggestion.email} · confirm before linking</small> : null}</span><button className="text-button" onClick={() => { setEditingIdentity(speaker); setIdentityEmail(identity?.email ?? suggestion?.email ?? ""); }}>Confirm email</button>{editingIdentity === speaker ? <div className="speaker-identity-form"><label>Email for {speaker}<input type="email" list="source-invitee-emails" value={identityEmail} onChange={(event) => setIdentityEmail(event.target.value)} placeholder="person@company.com" /></label><datalist id="source-invitee-emails">{source?.invitees?.filter((person) => person.email).map((person) => <option key={person.email} value={person.email ?? ""}>{person.name}</option>)}</datalist><div className="dialog-actions"><button className="button secondary" onClick={() => setEditingIdentity(null)}>Cancel</button><button className="button primary" onClick={() => void saveIdentity(speaker)}>Save mapping</button></div></div> : null}</div>; })}</div> : null}{meeting.errorMessage ? <p className="inline-error"><b>Adapter message:</b> {meeting.errorMessage}</p> : null}</section>
    </div>
