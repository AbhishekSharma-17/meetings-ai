...
| 4. Team operations | Calendar scheduling, shared templates, delivery policies, retention controls | Manual meeting deletion and opt-in scheduled retention are built; durable multi-replica queue, alerts and backup drills remain | Partial |
| 5. Agentic knowledge | Named bases, controlled sharing, saved chats, model selection, source-linked Q&A, evidence map and linked meetings | Canonical records, durable background indexing and bounded query planning are built; full topic/person pages and retrieval evaluation remain | Partial |
