...
| 5. Agentic knowledge | Named bases with confirmed deletion, meeting assignment, private/org/specific sharing, saved chats with per-user export/delete, selectable text model, source-linked Q&A, literal topic/speaker evidence map | Canonical records plus manually refreshed, organization-scoped hybrid index are built; background indexing, query planning, full linked pages and automatic retention remain | Source-linked hybrid workflow built; deeper agent planned |
