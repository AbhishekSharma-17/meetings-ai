...
| 5. Agentic knowledge | Named bases, meeting assignment, private/org/specific sharing, saved chats, selectable text model, source-linked Q&A | Current retrieval reads canonical records; OpenAI and compatible embeddings adapters are wired, while semantic indexing, re-index/delete, query planning and linked topic pages remain | Source-linked lexical workflow plus embedding runtime built; deeper agent planned |
