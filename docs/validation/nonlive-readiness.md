...
- A vector logo, meeting tags, opt-in AI knowledge settings, source-linked
  lexical search, and provider-agnostic draft Q&A are implemented and covered
...
- Named knowledge bases, per-base text-model selection, saved chats, local
  ...
  only one organization; no customer-to-customer isolation is claimed.
...
- Semantic/vector retrieval, query-planning agents, linked topic/person pages,
  and organization-scoped indexing/deletion are not implemented yet.
